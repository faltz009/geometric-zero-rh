...
$\mathrm{Re}(s) = 1/2$, and the functional equation, acting as a mirror
symmetry on the sphere, locks every geometric zero to this line. The Riemann
Hypothesis is a consequence of the geometry: every algebraic zero lies on
$\mathrm{Re}(s) = 1/2$ because the sphere permits geometric zeros nowhere else.
...
There is a framing asymmetry worth naming. Calling Definition~B a
``foundational claim rather than a proved theorem'' measures it against the
classical baseline, taking the $\mathbb{C}$ notion of zero as prior and asking
whether the $S^3$ notion agrees with it. The same standard applied to the
classical framework produces an equally open question: why does the
$\mathbb{C}$ shadow of the four-dimensional Euler product preserve all the
relevant structure? That question has no proof. The assumption that
$\mathbb{C}$ is sufficient is the hidden foundational claim of classical
analysis, carried silently because it is the default.

Lagrange's theorem removes the asymmetry. Primes live on $S^3$ by arithmetic
necessity, not by choice: the four-square decomposition is forced. The Euler
product is a composition on $S^3$. The $\mathbb{C}$ framework projects this
object to a two-dimensional shadow and works with the shadow. The $S^3$
framework works with the object itself. Definition~B is explicit about what
it assumes. The classical framework is not.
