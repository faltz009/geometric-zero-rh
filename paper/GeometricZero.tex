...
three geometric axioms and one definitional statement, all geometric properties
of the Hurwitz Euler product, and the classical Riemann Hypothesis follows in
one line.
...
With the geometry of $S^3$ established, the central definition of this paper
follows naturally.
...
The classical zero --- the point where the complex Euler product $\zeta(s) = 0$
--- is the two-dimensional shadow of this geometric event. When the running
product $Q(s)$ reaches Hopf balance on $S^3$, its projection onto
$\mathbb{C} \subset \mathbb{H}$ records a specific value; the classical zero
is the statement that this value is zero. The geometric zero is the event; the
classical zero is the shadow.
...
The choice of name matters: this is a definition, because it describes what a
zero of $\zeta$ \emph{is} in the natural geometry of the problem. The classical
formulation of zeros as complex values where a function vanishes is the same
definition after projection to flat space.
...
computations use the Hurwitz--$s$ lift with primes up to 5000, and source
code is in \texttt{experiments/primes\_on\_s3/}.
